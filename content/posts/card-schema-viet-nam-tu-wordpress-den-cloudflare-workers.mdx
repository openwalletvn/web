---
title: "Card schema cho thẻ tín dụng Việt Nam: từ SQL-in-WordPress đến JSON files trên edge"
description: >-
...
Sau đó chuyển sang các dự án Next.js khác, tích lũy thêm kinh nghiệm, rồi quay lại xây OpenWallet từ đầu với kiến trúc hoàn toàn khác. tich.vn chính thức deprecated.

### Vấn đề của WPGraphQL

WordPress là tool đúng cho blog. Nó là tool sai cho data API với business logic phức tạp.

WPGraphQL thêm overhead cả về runtime lẫn mental model. Taxonomy term không phải schema, nó là label. Khi cần validate data, không có contract nào cả, bất kỳ field nào cũng có thể có bất kỳ shape nào. GraphQL tốt khi client cần chọn field linh hoạt theo từng query. Nhưng với card comparison, ranking, và cashback calculation là các computed operation phức tạp, REST endpoint với typed response rõ ràng và dễ cache hơn nhiều.
...
API chạy trên Cloudflare Workers. Lý do đầu tiên rất thực tế: Cloudflare cho phép deploy từ private GitHub organization repository ở plan miễn phí. Vercel chỉ cho phép private personal repo. OpenWallet là org repo, nên đây là yếu tố quyết định ngay từ đầu.
