---
title: "Vì sao OpenWallet bỏ Headless WordPress để chuyển sang JSON cho dữ liệu thẻ"
description: >-
  ...
  Quá trình thay đổi để đáp ứng từng thể lệ hoàn tiền của từng thẻ.
date: '2026-06-08'
cover_image: /images/posts/vi-sao-openwallet-bo-headless-wordpress-de-chuyen-sang-json-cho-du-lieu-the/openwallet-card-json.png
category: Case study
status: published
---

Khi bắt đầu xây dựng OpenWallet, câu hỏi đầu tiên của mình không còn là "_dùng framework nào_" mà là "_làm sao lưu
thông tin một cái thẻ tín dụng cho đủ và đúng._" Nghe có vẻ đơn giản thôi, mình cũng từng nghĩ vậy cho đến khi
phải dừng lại và quyết định drop project với tổng cộng hơn 500 commits + 268 working hours.
Bài này kể lại hành trình đó, từ điểm xuất phát đến kiến trúc đang chạy hôm nay.

<BlogInlineImage src="/images/posts/vi-sao-openwallet-bo-headless-wordpress-de-chuyen-sang-json-cho-du-lieu-the/tichvn-tmetric.png" alt="268 giờ làm việc trên tich.vn" />

## Bắt đầu với tich.vn và WordPress

Trước OpenWallet là tich.vn, một dự án tra cứu thẻ, tra cứu MCC xây dựng trên nền tảng Headless WordPress CMS. 
Data thẻ lưu trong SQL database của WP thông qua ACF (Advanced Custom Fields) và Custom Post Type. 
WPGraphQL đứng ở giữa để Next.js có thể query dữ liệu qua GraphQL. 
Lúc đó WPGraphQL trông khá fancy (ít nhất là với mình), và đó cũng là lý do chính để chọn nó, 
nhưng nhìn lại thì đó là một lựa chọn đặt product vào stack mình muốn chứ không phải chọn stack phù hợp với product.
...
Cashback lúc đó nằm trong ACF fields của một CPT riêng (`cpt_reward`), liên kết với thẻ qua GraphQL resolver. 
MCC code lưu dưới dạng string `"3000, 3001, 3002"` trong một ACF field, 
rồi query bằng `meta_query REGEXP` để tìm ưu đãi nào chứa mã MCC đó.
...
Phần cashback không có trong JSON này mà nằm ở WPGraphQL, cần query riêng. 
Mỗi lần cập nhật một thẻ cần sửa trong WP admin, revalidate Next.js từ WP admin. Process khá là rườm rà.

### Vấn đề của WordPress Headless CMS

![WordPress Headless CMS on Google order](/images/posts/vi-sao-openwallet-bo-headless-wordpress-de-chuyen-sang-json-cho-du-lieu-the/headless-wp.png)

Không phải vì WordPress tệ. Mà vì cấu trúc này khó để scale khi số lượng thẻ tăng và cashback rules trở nên phức tạp hơn. 
Mỗi lần thêm loại rule mới cần sửa ACF field group, sửa GraphQL resolver, sửa Next.js query, 
deploy cả WP lẫn frontend. Và workflow này cũng **không thân thiện với AI coding assistant** - thứ mà một solo engineer như mình sử dụng mỗi ngày.
Khi API không theo kịp tốc độ cần thiết để thêm thẻ mới và model cashback đúng, dự án bị đình lại.

Sau đó mình chuyển sang các dự án Next.js khác, tích lũy thêm kinh nghiệm, 
rồi quay lại xây OpenWallet từ đầu với kiến trúc hoàn toàn khác. Tháng 6/2025, tich.vn chính thức deprecated, khi OpenWallet đã
bắt đầu đủ ổn định.

### Vấn đề của GraphQL

WordPress là tool đúng cho blog hoặc commercial website. 
Nhưng nó lại là tool sai cho data API với business logic phức tạp. 
Mặc dù bản thân Automattic cũng cố gắng tích hợp RESTful API, GraphQL API (bằng một plugin của bên thứ ba là [WPGraphQL](https://github.com/wp-graphql/wp-graphql)) vào Wordpress nhưng thật sự vẫn chỉ là những 
chắp vá ở phần ngọn.

WPGraphQL thêm overhead cả về runtime lẫn mental model. Taxonomy term không phải schema, nó là label. 
Khi cần validate data, không có contract nào cả, không thể chạy test, bất kỳ field nào cũng có thể có bất kỳ shape nào. GraphQL tốt khi client cần chọn field linh hoạt theo từng query. Nhưng với card comparison, ranking, và cashback calculation là các computed operation phức tạp, REST endpoint với typed response rõ ràng và dễ cache hơn nhiều.

## Quyết định rebuild: JSON files thay vì SQL, REST thay vì GraphQL

### Tại sao bỏ relational database như SQL?

Card data không có transaction. Không có concurrent write. Không có user session. 
"Relational" duy nhất thật sự cần là hai quan hệ: `bank_id` trỏ đến bank, `card_network` trỏ đến network entity. 
Hai foreign key này xử lý được bằng enrichment tại generate time, kết quả bundle vào JSON trước khi deploy.

SQL có nghĩa là server, connection pool, migration files, backup strategy. 
Với dữ liệu của 23 ngân hàng và 321 thẻ, đây là overhead không cần thiết.
...
Đây là điểm ảnh hưởng lớn nhất đến cách làm việc hàng ngày của dev: toàn bộ API server chạy được hoàn toàn local, kể cả database.
...
Khi chọn hosting, câu hỏi không phải "platform nào tốt hơn" mà là "_**platform nào phù hợp nhất cho từng phần, với cost thấp nhất**_."
...
- **`wrangler deploy` đơn giản:** gần tương đương `git push`, không cần quản lý server hay infrastructure riêng
...
> Nội dung từ phần này trở xuống được tổng hợp bởi LLM.
...
Hiện tại OpenWallet API là một private repo và mình chưa có kế hoạch public nó trong thời gian gần, tuy nhiên mình rất sẵn lòng add mọi người vào repo để
tham khảo. Liên hệ mình qua discord nhé.
