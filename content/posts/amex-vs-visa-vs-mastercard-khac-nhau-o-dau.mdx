...
description: >-
  Amex, Visa và Mastercard không chỉ khác nhau ở số chữ số. Bài viết phân tích
  cấu trúc hệ thống, phí merchant, tỷ lệ chấp nhận và khi nào nên chọn loại nào.
date: '2026-03-05'
...
ai_generated: true
cover_image: >-
  /images/posts/amex-vs-visa-vs-mastercard-khac-nhau-o-dau/ba-mang-luoi-the-amex-visa-mastercard.webp
image_prompts:
  ba-mang-luoi-the-amex-visa-mastercard: >
    A stylized flat illustration of three real Vietnamese bank credit cards
    fanned out in a cascade, each tilted at a slightly different angle and
    overlapping.

    Left card - a Techcombank Visa card: bright red body (#e30613), white
    "Techcombank" wordmark along the top, 16-digit number in 4-4-4-4 groups
    embossed across the middle, small "VISA" logotype in white at bottom-right.
    Tilted slightly left.

    Center card - a VPBank American Express card: deep green body (#00a650) with
    gold trim accent along the bottom edge, "VPBank" wordmark in white at top,
    15-digit number in 4-6-5 groups across the middle, "AMERICAN EXPRESS"
    wordmark in gold at bottom-right. Positioned slightly forward and raised
    above the others - the hero card.

    Right card - an MB Bank Mastercard: dark navy body (#004f9f), white "MB"
    mark at top-left, 16-digit number in 4-4-4-4 groups, two overlapping circles
    (red #eb001b and orange #f79e1b) Mastercard symbol at bottom-right. Tilted
    slightly right.

    Background: soft gradient from light blue (#dbeafe) top-left to white
    (#f8fafc) bottom-right, with a very faint hexagon mesh pattern at 10%
    opacity. Each card has a subtle drop shadow and slight gloss highlight on
    the top edge. No human figures. High detail, professional fintech
    illustration.
  mo-hinh-open-loop-closed-loop-visa-amex: >
    A side-by-side two-panel flow diagram on a white background showing two
    payment network models.

    Left panel - "Open-loop (Visa / Mastercard)": Four rounded rectangle nodes
    arranged in a square: top-left "Cardholder", top-right "Issuing Bank",
    bottom-right "Acquiring Bank", bottom-left "Merchant". In the center, a
    small circle labeled "Network". Arrows flow: Cardholder → Issuing Bank →
    Network → Acquiring Bank → Merchant. A dashed return arrow from Merchant
    back to Cardholder (goods/services). Color: blue theme (#2563eb accents).

    Right panel - "Closed-loop (American Express)": Three nodes only: top
    "Cardholder", bottom-left "American Express" (larger, central diamond
    shape), bottom-right "Merchant". Arrows: Cardholder ↔ Amex (billing,
    rewards), Amex ↔ Merchant (settlement, contract). Amex node is gold
    (#f59e0b). Much simpler flow than the left panel. Color: gold/amber theme.

    Background of both panels: very light gradient (#f8fafc → #f1f5f9). Clean
    sans-serif labels, thin arrow lines, flat design, professional. Text labels
    must be legible and accurate.
  phan-khuc-cao-cap-amex-vs-dai-chung-visa-mastercard: >
    A vertical pyramid / tier illustration on a soft gradient background
    (#e0f2fe → #f8fafc) with a faint diagonal line pattern at 8% opacity.

    Three horizontal bands stacked from wide at the bottom to narrow at the top,
    representing market segments from mass-market to super-premium.

    Bottom band (widest, light blue #bfdbfe): the broadest segment - many
    generic card shapes in blue and red tones, tightly packed, representing
    everyday consumer cards.

    Middle band (medium width, blue #60a5fa): fewer cards, more spaced out, in
    richer colors - gold and deep blue tones - representing premium cards.

    Top band (narrowest, dark navy #1e40af with a gold shimmer trim): just 1–2
    card shapes in gold/black, representing super-premium. A small gold star or
    crown above this band.

    A vertical arrow on the left side of the pyramid pointing upward, labeled
    "Premium level →". On the right side, a label at the bottom says "Visa &
    Mastercard" spanning all three bands with a bracket. A separate label at the
    top says "Amex" pointing only at the top band.

    Flat design, no specific card names or numbers on the card shapes, clean and
    professional. Text labels must be legible.
card_slugs:
  - vib-family-link
  - vib-super-card
  - vib-travel-elite
