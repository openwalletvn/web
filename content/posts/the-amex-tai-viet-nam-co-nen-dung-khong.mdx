...
status: "published"
ai_generated: true
cover_image: "/images/posts/the-amex-tai-viet-nam-co-nen-dung-khong/the-amex-viet-nam-quyen-loi-san-bay-phong-cho.webp"
image_prompts:
  the-amex-viet-nam-quyen-loi-san-bay-phong-cho: >
    [CARD REFERENCE: upload the Sacombank American Express card photo or VIB Super Card photo alongside
    this prompt so Gemini uses the exact card design — otherwise approximate: Sacombank Amex uses
    orange/red tones with gold trim; VIB Super Card uses deep purple (#6b21a8) body with gold accents,
    VIB wordmark in white, 15-digit number in 4-6-5 format, American Express in gold at bottom.]

    A premium flat illustration of a Vietnamese Amex card placed at an angle on a surface,
    slightly tilted, with a subtle drop shadow and gloss highlight on the top edge.
    Behind the card, a soft-lit airport lounge scene rendered as a blurred flat illustration:
    large panoramic windows, lounge chairs in warm tones, a miniature plane silhouette visible
    through the window. The lounge background is impressionistic — just enough detail to convey
    "premium airport lounge" without distracting from the card in the foreground.
    Background gradient: deep teal (#0d4f4f) at edges fading to soft warm cream (#f5f0e8) at center.
    Faint diagonal line pattern at 10% opacity on background.
    No text, no human figures, no logos beyond what is on the card itself. High detail, fintech illustration.

  ty-le-chap-nhan-amex-viet-nam-vs-visa-mastercard: >
    A creative split comparison illustration showing payment card acceptance contrast in Vietnam.

    Left half — "Visa / Mastercard": a dense cluster of small shop icons (coffee cup, grocery bag,
    petrol pump, shopping cart, phone, restaurant bowl) filling most of the space, each with a
    small green checkmark. Feels abundant, ~20+ icons. Color: blue tones (#2563eb).

    Right half — "American Express": the same set of shop icons but most are grayed out (#d1d5db)
    with no checkmark. Only 4–5 icons remain colored with a green checkmark: a hotel building,
    an airplane, a luxury shopping bag, a laptop (international online), a shopping cart with
    "Shopee/Tiki" label (domestic ecommerce). Feels sparse but includes key ecommerce.

    A thin vertical divider between the two halves. Above left: "Visa / Mastercard — ~95% POS".
    Above right: "Amex — ~15–25% POS vật lý". Both labels in clean sans-serif.

    Background: very light gray (#f8fafc) with a faint dot-grid pattern. Flat icon style, no
    human figures, text labels must be legible and accurate.

  doi-tuong-phu-hop-dung-the-amex-viet-nam: >
    A clean two-column "fit / no fit" illustration on a white background with light gray card containers.

    Left column — green header bar (#16a34a), title "Phù hợp":
    4 icon + short label rows, each with a green checkmark circle:
    ✓ Airplane icon — "Thường xuyên bay quốc tế"
    ✓ Globe + laptop icon — "Chi tiêu chủ yếu trên nền tảng quốc tế"
    ✓ Two-card stack icon — "Đã có thẻ Visa/MC làm thẻ chính"
    ✓ Calculator icon — "Quyền lợi vượt phí thường niên"

    Right column — red header bar (#dc2626), title "Không phù hợp":
    4 icon + short label rows, each with a red X circle:
    ✗ Shopping cart + bowl icon — "Chủ yếu chi tiêu nội địa hàng ngày"
    ✗ Single card icon — "Muốn một thẻ duy nhất cho tất cả"
    ✗ Calendar with X icon — "Ít đi lại quốc tế"
    ✗ Scale/compare icon — "So sánh với Visa Infinite / MC World Elite"

    Layout: two tall rounded-rectangle cards side by side. Clean sans-serif labels, icon style
    consistent with flat fintech design. Background: soft gradient #f8fafc → #f1f5f9.
