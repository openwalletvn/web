...
import { getNetworks, getCards, getNetworkImageUrl } from '@/lib/api';
...
export default async function NetworksIndexPage() {
  const [networks, allCards, t, tb] = await Promise.all([
    getNetworks(),
    getCards(),
  ...
  const cardCountByNetwork = new Map<string, number>();
  for (const card of allCards) {
    if (card.card_network) {
      cardCountByNetwork.set(card.card_network, (cardCountByNetwork.get(card.card_network) ?? 0) + 1);
    }
  }
  ...
  return (
    <div className="px-4 py-12">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-slate-500 mb-8">{t('count', { count: networks.length })}</p>
