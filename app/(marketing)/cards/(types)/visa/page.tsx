...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: `${cardsPage('network_visa')} | Open Wallet`,
    alternates: { canonical: '/cards/visa' },
  };
}

export default async function VisaCardsPage() {
      ...
      <div className="max-w-6xl mx-auto">
        <Breadcrumbs
          items={[
            ...
            { label: cardsPage('network_visa') },
          ]}
        />
        <h1 className="text-4xl font-bold text-slate-900 mb-6">{cardsPage('network_visa')}</h1>
        <Suspense fallback={<CardsSectionSkeleton />}>
          <CardsSection filters={{ network: 'visa' }} title="" />
        </Suspense>
      </div>
    </div>
  );
