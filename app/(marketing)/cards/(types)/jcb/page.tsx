...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: `${cardsPage('network_jcb')} | Open Wallet`,
    alternates: { canonical: '/cards/jcb' },
  };
}

export default async function JcbCardsPage() {
      ...
      <div className="max-w-6xl mx-auto">
        <Breadcrumbs
          items={[
            ...
            { label: cardsPage('network_jcb') },
          ]}
        />
        <h1 className="text-4xl font-bold text-slate-900 mb-6">{cardsPage('network_jcb')}</h1>
        <Suspense fallback={<CardsSectionSkeleton />}>
          <CardsSection filters={{ network: 'jcb' }} title="" />
        </Suspense>
      </div>
    </div>
  );
