...
export async function generateStaticParams() {
  try {
    ...
    return json.data.map((brand: { id: string }) => ({ slug: brand.id }));
  } catch {
    return [];
  }
}
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const res = await fetch(`${apiUrl}/api/v1/brands/${slug}`);
    const json = await res.json();
    if (!json.success) throw new Error('not found');
    const brand = json.data;

    const logoUrl = brand.logo_url ? `${apiUrl}${brand.logo_url}` : null;
    ...
    return createOgImage({
      title: brand.name,
      description: `Co-branded cards · Open Wallet`,
    ...
    return createOgImage({ title: slug, description: 'Open Wallet · Co-branded' });
  }
}
