...
export default async function HomePage() {
  const [t, tm, torg] = await Promise.all([
  ...
  const jsonLd = {
    ...
    '@graph': [
      ...
      },
      {
        ...
        name: tm('siteName'),
        url: 'https://openwallet.vn',
        logo: 'https://openwallet.vn/logo.svg',
        ...
        <div className="max-w-4xl mx-auto w-full text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-12">
            ...
            <div className="bg-white border border-brand-red rounded-xl p-6 text-left flex flex-col shadow-sm">
              <h2 className="text-xl font-bold text-slate-900 mb-2">{t('db.title')}</h2>
              <p className="text-slate-500 mb-6 flex-1">{t('db.description')}</p>
              <Link
                href="/docs"
            ...
            <div className="bg-white border border-brand-blue rounded-xl p-6 text-left flex flex-col shadow-sm">
              <div className="flex items-center gap-2 mb-2">
                <h2 className="text-xl font-bold text-slate-900">{t('app.title')}</h2>
                <span className="text-xs px-2 py-0.5 bg-brand-blue/10 text-brand-blue rounded-full font-medium">
                  {t('app.badge')}
                </span>
              </div>
              <p className="text-slate-500 mb-6 flex-1">{t('app.description')}</p>
