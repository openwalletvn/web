...
export default async function DichVuSoCardsPage() {
    const [allCards, initialRanked] = await Promise.all([
        getCards(),
        getRankedCards({spend: {digital: 3_000_000}, limit: 50}).catch(() => []),
    ]);
    const cards = allCards.filter((c) => c.intents?.includes('digital'));
    ...
    return (
        <MarketingPageShell title={TITLE} description={DESCRIPTION} breadcrumbItems={breadcrumbItems} jsonLd={jsonLd}>
            ...
            <CardRankingTable initialRanked={initialRanked} intentSlug="digital" title="Xếp hạng thẻ theo cashback dịch vụ số"/>
