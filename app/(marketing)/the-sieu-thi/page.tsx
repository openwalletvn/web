...
export default async function SieuThiCardsPage() {
    const [allCards, initialRanked] = await Promise.all([
        getCards(),
        getRankedCards({spend: {groceries: 3_000_000}, limit: 50}).catch(() => []),
    ]);
    const cards = allCards.filter((c) => c.intents?.includes('groceries'));
    ...
    return (
        <MarketingPageShell title={TITLE} description={DESCRIPTION} breadcrumbItems={breadcrumbItems} jsonLd={jsonLd}>
            ...
            <CardRankingTable initialRanked={initialRanked} intentSlug="groceries" title="Xếp hạng thẻ theo cashback siêu thị"/>
