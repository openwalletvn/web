...
export function CardSelectionStep({
  ...
  return (
    <div className="ow-card-selection-step">
      <div className="flex items-center gap-2 mb-5 p-2.5 border border-dashed border-slate-200 rounded w-fit">
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="border border-dashed border-slate-200 rounded p-3">
              <div className="w-full aspect-[16/10] bg-slate-100 rounded animate-pulse mb-2" />
          ...
          {cards.map((card) => {
            ...
            return (
              <button
                ...
                className="flex flex-col gap-2 p-3 border border-dashed border-slate-200 rounded hover:border-brand-blue hover:bg-blue-50/40 transition-colors text-left"
              >
                ...
                <div className="flex items-start justify-between gap-1.5 min-w-0">
                  ...
                  {owned && (
                    <span className="shrink-0 text-xs text-green-600 border border-dashed border-green-300 px-1 py-0.5 rounded leading-none mt-0.5">
                      Đã có
                    </span>
                  )}
                </div>
