...
export function OwSourceList({sources}: Props) {
    ...
    return (
        <div className="ow-source-list flex flex-col gap-2">
                        ...
                        <div className="flex flex-col gap-0.5">
                            <div className="flex flex-wrap items-center gap-1">
                                ...
                                {src.url.toLowerCase().endsWith('.pdf') && (
                                    <span className="text-xs font-semibold tracking-wide inline-block text-red-500 border border-red-300 rounded px-1 py-0.5 leading-none">PDF</span>
                                )}
                                <IconExternalLink className="w-4 h-4"/>
                            </div>
                            {src.label && (
                                <span className="text-xs text-slate-400 line-clamp-2 break-all">{urlFallbackLabel(src.url)}</span>
                            )}
                        </div>
                    </li>
                ))}
