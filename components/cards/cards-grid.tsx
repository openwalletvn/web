...
function CardsGridInner({
    ...
    const df = useMemo(() => new URLSearchParams(defaultFilters ?? ''), [defaultFilters]);

    // Local state (used when useUrlState=false)
    const [localType, setLocalType] = useState<string | null>(df.get('type'));
    const [localNetwork, setLocalNetwork] = useState<string | null>(df.get('network'));
    const [localBankId, setLocalBankId] = useState<string | null>(df.get('bank'));
    const [localCoBrand, setLocalCoBrand] = useState<string | null>(df.get('co_brand'));
    const [localSort, setLocalSort] = useState<string | null>(df.get('sort'));
    const [localContactless, setLocalContactless] = useState<string | null>(df.get('contactless'));
    const [localFee, setLocalFee] = useState<string | null>(df.get('fee'));
    const [localTier, setLocalTier] = useState<string | null>(df.get('tier'));

    useEffect(() => {
        if (!useUrlState || !defaultFilters) return;
        ...
        <div className="ow-cards-grid">
            ...
            <div className="">
                {displayed.length === 0 ? (
                    ...
                    <CardMasonry cards={displayed} maxDisplay={max}/>
                )}
            </div>
