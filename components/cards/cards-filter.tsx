    ...
    <div className={`ow-cards-filter space-y-3 transition-opacity${isPending ? ' opacity-60 pointer-events-none' : ''}`}>
      ...
      {hasFilter && (
        <div className="flex gap-2 flex-wrap items-center">
          {activeType && (
            <FilterChip
              label={activeType === 'credit' ? 'Tín dụng' : activeType === 'debit' ? 'Ghi nợ' : activeType === 'prepaid' ? 'Trả trước' : activeType === 'transit' ? 'Transit' : activeType === 'atm' ? 'ATM' : activeType}
              onRemove={() => update('type', 'all')}
            />
          )}
          {activeNetwork && (
            <FilterChip
              label={networkChipData?.name ?? activeNetwork}
          ...
          {fee && (
            <FilterChip
              label={fee === 'free' ? 'Miễn phí' : fee === 'unknown' ? 'Không rõ phí' : (FEE_BUCKETS.find((b) => b.value === fee)?.label ?? fee)}
              onRemove={() => update('fee', 'all')}
            />
          )}
          {sortValue !== 'default' && (
            <FilterChip
              label={sortValue === 'fee_asc' ? 'Phí thường niên ↑' : sortValue === 'fee_desc' ? 'Phí thường niên ↓' : 'Hạng thẻ (cao → thấp)'}
